...
class RealTimeInput(ParentClass):
    ...
    def get_input(self, return_raw_key=False):
        ...
        def Linux():
            ...
            filedescriptors = params.termios.tcgetattr(sys.stdin)
            params.tty.setcbreak(sys.stdin)
            key = sys.stdin.read(1)[0]
            params.termios.tcsetattr(
                sys.stdin, params.termios.TCSADRAIN, filedescriptors)
            return key
        ...
        def Windows():
            ...
            while True:
                if params.msvcrt.kbhit():  # key is pressed
                    key = params.msvcrt.getwch()  # decode
                    return key
    ...
    def prepare_autocomplete(self, **kwargs):
        ...
        if len(self.suggestions) == 0:
            ...
            self.display = '{string} - ({i}/{n}) - {suggestion}'.format(string=self.string, i=self.suggestion_index +
                                                                        1, n=len(self.suggestions), suggestion=self.suggestions[self.suggestion_index])
    ...
    def get_one_input(self, search_kwargs={}, autocomplete_kwargs={}):
        ...
        while True:
            ...
            if len(self.suggestions) > 0:
                self.suggestion_index = self.suggestion_index % len(
                    self.suggestions)
